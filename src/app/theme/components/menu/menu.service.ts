...
export class MenuService {
  ...
  private closeOtherSubMenus(elem) {
    const elemId = elem.offsetParent.children[0].id
    const children = this.renderer2.parentNode(elem.offsetParent).children
    for (let i = 0; i < children.length; i++) {
      const child = this.renderer2.nextSibling(children[i].children[0])
      if (child && elemId !== children[i].children[0].id) {
